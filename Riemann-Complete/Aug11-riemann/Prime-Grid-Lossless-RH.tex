...
\begin{abstract}
We develop a bounded-real (Herglotz/Schur) formulation of the Riemann Hypothesis (RH) on the right half-plane \(\Omega:=\{\Re s>\tfrac12\}\). Let \(A(s):\ell^2(\PP)\to\ell^2(\PP)\) be the prime-diagonal operator \(A(s)e_p:=p^{-s}e_p\). With the Hilbert--Schmidt regularized determinant \(\dettwo\) and the completed zeta \(\xi(s)\), we set \(J(s):=\dettwo(I-A(s))/\xi(s)\) and adopt the Cayley transform \(\Theta(s):=\big(2J(s)-1\big)\big/\big(2J(s)+1\big)\), so \(\Theta(\sigma+it)\to -1\) as \(\sigma\to+\infty\). Our method hinges on: (i) a Schur–determinant splitting that isolates the \(k=1\) and archimedean terms into a finite block; (ii) Hilbert–Schmidt control of prime truncations implying local-uniform convergence of \(\dettwo(I-A_N)\); (iii) explicit finite-stage passive realizations certified by the Kalman–Yakubovich–Popov (KYP) lemma; and (iv) an interior passive \(H^\infty\) approximation scheme on zero-free rectangles.

From these ingredients we construct Schur approximants that converge locally uniformly to \(\Theta\) on \(\Omega\setminus S\) for a discrete set \(S\), and show that the limit extends holomorphically across \(S\) with \(|\Theta|\le 1\) on all of \(\Omega\). A one-line maximum-modulus argument then excludes zeros of \(\xi\) in \(\Omega\), proving RH. The boundary route and its positivity hypothesis are not required for this interior completion.
\end{abstract}
...
\item \textbf{HS continuity for \(\dettwo\).} Prime truncations \(A_N\to A\) in the HS topology, uniformly on compacts in \(\Omega\), imply local-uniform convergence of \(\dettwo(I-A_N)\). Division by \(\xi\) is justified only on compacts avoiding its zeros; throughout we explicitly state such hypotheses when needed.
 ...
 \item \textbf{Interior passive \(H^\infty\) approximation and extension.} On zero-free rectangles we build Schur rational approximants converging locally uniformly to \(\Theta\); a removable-singularity argument eliminates internal poles (\(P=\{2J=-1\}\)) and extends \(\Theta\) holomorphically to \(\Omega\) with \(|\Theta|\le 1\).
\end{itemize}
From these ingredients we obtain a global Schur limit \(\Theta\) on \(\Omega\). A one-line maximum-modulus argument then excludes zeros of \(\xi\) in \(\Omega\), and RH follows by the functional equation.
...
\begin{theorem}[Uniform $L^1_{\mathrm{loc}}$ and Cauchy as \(\varepsilon\downarrow 0\) under (D)]\label{thm:uniform-eps}
Assume (D). For every compact interval $I\subset\R$ there exist constants $C_I$ and \(\varepsilon_0>0\) such that for all \(\varepsilon\in(0,\varepsilon_0)\),
\[
...
Let \(K\Subset R\Subset R^\sharp\Subset\Omega\) with \(|\xi|\ge \delta_R>0\) on \(R^\sharp\). For \(N\) large, define \(g_N:=\Theta_N^{(\dettwo)}\) on \(\partial R\). Then there exist lossless (Schur) rationals \(\Theta_{N,M}\) of McMillan degree \(\le M\), which are Schur on the whole half-plane \(\Omega\), with
\[
...
\]
Pull back \(M_1 s_M\) to \(\partial R\) via the Cayley map (disk \(\leftrightarrow\) half-plane) to obtain a Schur function \(\Theta_{N,M}\) on \(\partial R\) with
\[
 \sup_{\partial R}\,|\Theta_{N,M}-g_N|\ \le\ C(R,R^\sharp)\,\rho^M.
\]
By the maximum principle (applied after mapping back to the half-plane), the same bound holds on \(K\Subset R\). The Schur property is preserved by the Schur algorithm and by the Möbius equivalence between the disk and half-plane, so each \(\Theta_{N,M}\) is lossless (Schur) on \(\Omega\) as claimed.
\end{proof}

\begin{lemma}[Global diagonal Schur sequence on $\Omega$]\label{lem:global-diagonal}
Let $\{K_m\}_{m\ge 1}$ be an exhaustion of $\Omega\setminus Z(\xi)$ by compacts. For each $m$ choose $N(m)$ large and then $M(m)$ large so that
\[
 \sup_{K_m}\,\big|\Theta_{N(m),M(m)}-\Theta\big|\ <\ 1/m.
\]
Then the sequence $\Phi_k:=\Theta_{N(k),M(k)}$ consists of Schur functions on $\Omega$ and converges locally uniformly to $\Theta$ on $\Omega\setminus Z(\xi)$.
\end{lemma}
\begin{proof}
Each $\Theta_{N(m),M(m)}$ is Schur on $\Omega$ by Proposition~\ref{prop:hinf-passive}. Given a compact $K\Subset \Omega\setminus Z(\xi)$, choose $m_0$ so that $K\subset K_{m_0}$; then for all $k\ge m_0$ we have $\sup_{K}|\Phi_k-\Theta|\le \sup_{K_{m_0}}|\Theta_{N(k),M(k)}-\Theta|< 1/k$. This is local uniform convergence.
\end{proof}

\begin{theorem}[Unconditional interior completion \(\Rightarrow\) RH]\label{thm:interior-completion}
With \(J=\dettwo(I-A)/\xi\) and \(\Theta=(2J-1)/(2J+1)\) as above, the interior passive \(H^\infty\) approximation (Proposition~\ref{prop:hinf-passive}), the local-uniform convergence of \(\Theta_N^{(\dettwo)}\to\Theta\) off \(Z(\xi)\) (Lemma~\ref{lem:cayley-cont}), and Corollary~\ref{cor:global-schur} imply that \(\Theta\) is holomorphic on \(\Omega\) with \(|\Theta|\le 1\) there. Consequently \(\xi\) has no zeros in \(\Omega\), and RH holds.
\end{theorem}
\begin{proof}
...
\[
 \sup_{\partial\mathbb D}\big|M_1 s_M-h\big|\ \le\ C''\,\rho^M.
\]
Pull back \(M_1 s_M\) to \(\partial R\) via the Cayley map to obtain a Schur function \(\Theta_{N,M}\) on \(\partial R\) with
\[
 \sup_{\partial R}\,|\Theta_{N,M}-g_N|\ \le\ C(R,R^\sharp)\,\rho^M.
\]
...
Let \(u_\varepsilon(t):=\log\big|\dettwo(I-A(\tfrac12+\varepsilon+it))/\xi(\tfrac12+\varepsilon+it)\big|\). By Theorem~\ref{thm:uniform-eps}, \(u_\varepsilon\) is uniformly bounded in \(L^1_{\mathrm{loc}}(\R)\) and Cauchy as \(\varepsilon\downarrow 0\), so \(u_\varepsilon\to u_0\) in \(L^1_{\mathrm{loc}}(\R)\). For each \(\varepsilon>0\), let \(\mathcal O_\varepsilon\) be the outer with boundary modulus \(e^{u_\varepsilon}\) on \(\Re s=\tfrac12+\varepsilon\), and set \(\mathcal J_\varepsilon:=\dettwo(I\!-\!A)/(\mathcal O_\varepsilon\,\xi)\). Then \(|\mathcal J_\varepsilon|\equiv 1\) on \(\Re s=\tfrac12+\varepsilon\), and \(\mathcal J_\varepsilon\to \mathcal J:=\dettwo(I\!-\!A)/(\mathcal O\,\xi)\) locally uniformly on \(\Omega\).
If, moreover, (P+) holds for the boundary data of \(\mathcal J\), then \(2\mathcal J\) is Herglotz in \(\Omega\), so \(\Theta\) is Schur by Theorem~\ref{thm:equivalences}. No claim is made that \(\mathcal O\equiv 1\) or that \(\Theta\) is boundary-unimodular.
...
\begin{corollary}[Equivalence]\label{cor:equivalence}
BRF for $H$ on $\Omega$ implies RH (Theorem~\ref{thm:brf-implies-rh}). Conversely, assuming RH and (P+) for $\mathcal J=\dettwo(I-A)/(\mathcal O\,\xi)$, BRF holds (Theorem~\ref{thm:rh-implies-brf}). In particular, BRF is equivalent to RH together with (P+).
\end{corollary}

\begin{lemma}[Outer phase as Hilbert transform]\label{lem:outer-phase-HT}
Let $u\in L^1_{\mathrm{loc}}(\R)$ with $\int_\R \frac{|u(t)|}{1+t^2}\,dt<\infty$, and let $O$ be the outer function on $\Omega=\{\Re s>\tfrac12\}$ defined by \eqref{eq:outer-formula} with boundary modulus $e^{u}$. Then the non-tangential boundary values satisfy
\[
 \log|O(\tfrac12+it)|=u(t)\quad\text{for a.e. }t\in\R,
\]
and there exists a real constant $c\in\R$ (determined by a phase normalization, e.g. $\Arg O(\sigma_\ast)\in(-\pi,\pi]$ at a fixed $\sigma_\ast>\tfrac12$) such that
\[
 \Arg O(\tfrac12+it)\;=\;\mathcal H[u](t)+c\quad\text{for a.e. }t\in\R,
\]
where $\mathcal H$ is the (one-dimensional) Hilbert transform on $\R$. In particular, for any two choices of phase normalization the difference is a global constant.
\end{lemma}
\begin{proof}[Proof sketch]
Set $U(\sigma,t):=(\log|O|)(\sigma+it)$; by definition \eqref{eq:outer-formula}, $U$ is the Poisson extension of $u$ to the half-plane, hence harmonic with non-tangential limit $u$ a.e. on the boundary. The harmonic conjugate $V$ (unique up to an additive constant) satisfies the Cauchy–Riemann equations in $\Omega$ and has boundary values given by the Hilbert transform of $u$. Since $\log O=U+iV$ is analytic on $\Omega$, we obtain the stated boundary limits for $\Arg O=V$ up to an additive constant. Standard references include Duren and Garnett (disk case) transported to the half-plane by Cayley.
\end{proof}
...
\begin{proposition}[Positive measures from KYP limits on rectangles]\label{prop:measure-limit-rectangle}
Let $R\Subset R^\sharp\Subset\Omega$ be rectangles with $|\xi|\ge\delta>0$ on $R^\sharp$. For each $N$ and $M$, let $\Theta_{N,M}$ be the lossless Schur approximants provided by Proposition~\ref{prop:hinf-passive} on $R$, and set $H_{N,M}:=\mathcal C^{-1}[\Theta_{N,M}]$. Then:
\begin{enumerate}
 \item Each $H_{N,M}$ is a (matrix-free) Herglotz function on $R$;
 \item There exists a finite positive Borel measure $\mu_{N,M}$ on the boundary $\partial R$ such that
 \[
  H_{N,M}(s)\;=\;\alpha_{N,M}\,+\,\beta_{N,M}s\,+\,\int_{\partial R}\!\frac{1+\zeta\,\overline{s}}{\zeta-\overline{s}}\,d\mu_{N,M}(\zeta)\quad(s\in R),
 \]
 with bounded affine part \((\alpha_{N,M},\beta_{N,M})\) (depending on the McMillan degree/normalization and uniformly controlled over $R$);
 \item If \(\Theta_{N,M}\to \Theta^{(\dettwo)}\) uniformly on compact subsets of $R$ (as in Corollary~\ref{cor:interior-alignment}), then along a subsequence \(\mu_{N_k,M_k}\) converges weak-* to a finite positive Borel measure $\mu_R$ on $\partial R$ and
 \[
  H_{N_k,M_k}\longrightarrow H^{(\dettwo)}\quad\text{locally uniformly on }R,
 \]
 where $H^{(\dettwo)}=\mathcal C^{-1}[\Theta^{(\dettwo)}]$ has the representation above with $\mu_R$ in place of $\mu_{N,M}$ and limiting affine part.
\end{enumerate}
\end{proposition}
\begin{proof}[Proof sketch]
Lossless Schur on $R$ implies $H_{N,M}=\mathcal C^{-1}[\Theta_{N,M}]$ is Herglotz on $R$ by Theorem~\ref{thm:equivalences}. By the Herglotz representation in simply connected domains with analytic boundary (transported from the disk), $H_{N,M}$ admits a Poisson–Stieltjes representation by a finite positive measure on \(\partial R\) together with a bounded affine term. Uniform convergence on compact subsets of $R$ and local boundedness of $H_{N,M}$ allow a weak-* subsequence extraction for \(\mu_{N,M}\), yielding \(\mu_R\) so that the Poisson integrals and affine parts converge to $H^{(\dettwo)}$. This is standard via Montel normality and Poisson kernel stability on analytic curves.
\end{proof}

\subsection{Boundary Pick-matrix certification of (P+): plan and scaffold}\label{subsec:pick-cert}
On a compact interval $I\subset\R$, define \(\mathcal J:=\dettwo(I-A)/(\mathcal O\,\xi)\) and \(\Theta:=\mathcal C[2\mathcal J]=(2\mathcal J-1)/(2\mathcal J+1)\). The following practical certificate is equivalent to the phase cone on $I$.

\begin{proposition}[Pick-matrix positivity \(\Rightarrow\) boundary cone on $I$]\label{prop:pick-implies-cone}
Fix $I=[T_1,T_2]$. For \(\sigma>0\) and nodes $t_j\in I$, set $s_j:=\tfrac12+\sigma+i t_j$. If for arbitrarily fine grids $\{t_j\}_{j=1}^n\subset I$; set $s_j=\tfrac12+\sigma+i t_j$. (2) Evaluate \(\Theta(s_j)=(2\mathcal J(s_j)-1)/(2\mathcal J(s_j)+1)\) with \(\mathcal J=\dettwo(I-A)/(\mathcal O\,\xi)\). (3) Assemble \(K_{jk}=(1-\Theta(s_j)\overline{\Theta(s_k)})/(s_j+\overline{s_k}-1)\) and check PSD (e.g., Hermitian symmetrize and ensure all eigenvalues \(\ge 0\) within a tolerance). (4) Refine the grid and decrease \(\sigma\); positivity for arbitrarily fine grids certifies (P+) on $I$. Cover \(\R\) by such intervals to globalize.

\paragraph{Python scaffold (verbatim).}
\begin{verbatim}
import numpy as np

def pick_matrix(theta_vals, s_vals):
    n = len(s_vals)
    K = np.zeros((n, n), dtype=complex)
    for j in range(n):
        for k in range(n):
            num = 1 - theta_vals[j] * np.conjugate(theta_vals[k])
            den = s_vals[j] + np.conjugate(s_vals[k]) - 1.0
            K[j, k] = num / den
    # Hermitian symmetrization for numerical stability
    return 0.5 * (K + K.conj().T)

# User must provide: J(s) = det2(I-A(s)) / (O(s) * xi(s)) and Theta(s)
# Example interface:
# def J(s: complex) -> complex: ...
...
\begin{remark}[Certification by exhaustion]
Choose an open cover of \(\R\) by intervals \(I_m\) avoiding critical-line ordinates. Certify (P+) on each \(I_m\) by Proposition~\ref{prop:pick-implies-cone}; then \(2\mathcal J\) is Herglotz a.e. on \(\R\), hence on \(\Omega\) by the Poisson integral, and \(\Theta\) is Schur.
\end{remark}

\paragraph{Pilot certifications on unit windows.}
Using the fast driver with det$_2$ product surrogate, prime cutoff $P_{\max}=2000$, precision \(\mathrm{dps}=60\), and the quick \((\sigma,n)\) schedule, we obtain Pick-PSD positivity on the unit intervals below (reported minimal eigenvalues across the quick schedule):
\begin{center}
\begin{tabular}{c|l}
Interval $I$ & \(\min\,\lambda_{\min}(K)\) across quick schedule \\ \hline
$[1,2]$ & $[9.25\cdot10^{-7},\ 1.17\cdot10^{-10},\ 3.31\cdot10^{-5}]$ \\
$[2,3]$ & $[9.62\cdot10^{-7},\ 1.21\cdot10^{-10},\ 3.43\cdot10^{-5}]$ \\
$[3,4]$ & $[9.93\cdot10^{-7},\ 1.26\cdot10^{-10},\ 3.59\cdot10^{-5}]$ \\
$[4,5]$ & $[8.28\cdot10^{-7},\ 1.04\cdot10^{-10},\ 2.95\cdot10^{-5}]$ \\
\end{tabular}
\end{center}
This demonstrates local (P+) certificates on a nontrivial slice of the boundary, consistent with the phase-cone scan. Globalization proceeds by exhaustion (Remark above). Higher $P_{\max}$, refined schedules, and the uniform-\(\varepsilon\) outer normalization (Theorem~\ref{thm:uniform-eps}) will systematically extend coverage.

\paragraph{Preliminary numerical certification on a coarse cover.}
Using the driver in \texttt{tools/pick\_certify.py} with $P_{\max}=5\times 10^4$ and no outer normalization (i.e. \(\mathcal O\equiv 1\) surrogate), we obtained the following statuses on a coarse cover of $[0,40]$ by length-$10$ intervals (Chebyshev grids, \(\sigma\in\{10^{-2},5\cdot10^{-3},2\cdot10^{-3},10^{-3},5\cdot10^{-4}\}\)):
\begin{center}
\begin{tabular}{c|c|l}
Interval $I$ & Status & reported \(\min\,\lambda_{\min}(K)\) across schedule \\ \hline
$[0,10]$ & OK & $[2.1\cdot10^{-1},3.0\cdot10^{-1},1.76,2.68,7.00]$ \\
$[10,20]$ & FAIL & $[-7.84\cdot10^{-2}]$ \\
$[20,30]$ & FAIL & $[-7.61\cdot10^{-3}]$ \\
$[30,40]$ & FAIL & $[-2.82\cdot10^{-6}]$ \\
\end{tabular}
\end{center}
The surrogate run without outer normalization is only illustrative; rigorous certification must use the outer \(\mathcal O\) built from the boundary $L^1$ limit (Theorem~\ref{thm:uniform-eps}). The same driver supports a Poisson-built outer per interval/schedule; robust certification will require tuned quadrature and smoothing consistent with the $L^1$ data.

\begin{theorem}[Global (P+) from local Pick-grid certification]\label{thm:pick-globalization}
Let \(\{I_m\}_{m\in\mathbb N}\) be an open cover of \(\R\) such that each \(I_m\) contains no ordinate of a critical-line zero of \(\xi\). Suppose that for every \(m\) and for arbitrarily fine grids \(\{t_j\}\subset I_m\) with \(\sigma\downarrow 0\), the Pick matrices
\[
 K^{(m)}_{jk}(\sigma)\ :=\ \frac{1-\Theta(s_j)\,\overline{\Theta(s_k)}}{s_j+\overline{s_k}-1},\qquad s_j=\tfrac12+\sigma+i t_j,
\]
are positive semidefinite. Then \emph{(P+)} holds a.e. on \(\R\), i.e.
\[
 \Re\big(2\mathcal J(\tfrac12+it)\big)\ \ge\ 0\quad\text{for a.e. }t\in\R.
\]
Consequently, by Theorem~\ref{thm:global-PSD}, \(\Theta\) is Schur on \(\Omega\) and the Pick kernel \(K_\Theta\) is PSD on \(\Omega\).
\end{theorem}
\begin{proof}
By Proposition~\ref{prop:pick-implies-cone}, the assumed PSD property on arbitrarily fine grids in each \(I_m\) implies the boundary phase-cone and thus (P+) a.e. on each \(I_m\). The exceptional ordinates form a countable set (critical-line zeros), hence have measure zero; taking the union over \(m\) gives (P+) a.e. on \(\R\). Apply Theorem~\ref{thm:global-PSD}.
\end{proof}

\paragraph{Numerical guardrails and certification schedule.}
For reliable PSD checks on \(I=[T_1,T_2]\):
\begin{enumerate}
 \item Use Hermitian symmetrization \(K\leftarrow\tfrac12(K+K^\ast)\) and monitor the minimal eigenvalue \(\lambda_{\min}(K)\).
 \item Adopt a decreasing schedule \(\sigma_\ell\downarrow 0\) and an increasing grid size \(n_\ell\uparrow\infty\); certify if \(\lambda_{\min}(K)\ge -\mathrm{tol}_\ell\) for all \(\ell\) with \(\mathrm{tol}_\ell\to 0\).
 \item Prefer Chebyshev-in-\(t\) nodes on \([T_1,T_2]\) for conditioning; optionally add endpoints.
 \item Validate stability by small random jitter of nodes and by refining both \(\sigma\) and the grid.
\end{enumerate}

\paragraph{Python driver (verbatim).}
\begin{verbatim}
import numpy as np

# User must implement J(s) and optionally provide Theta(s)
# J(s): det2(I - A(s)) / (O(s) * xi(s)) with outer O built from boundary L1 data
...
def pick_matrix(theta_vals: np.ndarray, s_vals: np.ndarray) -> np.ndarray:
    ...
    for j in range(n):
        for k in range(n):
            ...
            den = s_vals[j] + np.conjugate(s_vals[k]) - 1.0
            K[j, k] = num / den
    return 0.5 * (K + K.conj().T)  # Hermitian symmetrization
...
def certify_interval(T1: float, T2: float,
                     sigmas=(1e-2, 5e-3, 2e-3, 1e-3, 5e-4),
                     ns=(64, 96, 128, 192, 256), tol=1e-10) -> bool:
...
def certify_cover(intervals, **kwargs):
    results = []
    for (T1, T2) in intervals:
        ok = certify_interval(T1, T2, **kwargs)
        results.append({"interval": (T1, T2), "certified": ok})
        print(f"I=[{T1},{T2}] -> {'OK' if ok else 'FAIL'}")
    return results
\end{verbatim}

\begin{remark}[Alternative zero-side route]
On short intervals one can certify the phase-cone via Proposition~\ref{prop:local-cone} by proving the zero-side bound \(\int_I (-w')\,dt\le \pi/2\) (Remark~\ref{rem:mass-bound}). Either route suffices for Theorem~\ref{thm:pick-globalization}.
\end{remark}

\begin{corollary}[Maximum-modulus pinch]\label{cor:MMP-pinch}
If $\xi(\rho)=0$ for some $\rho\in\Omega$, then $\Theta(\rho)=1$ and $|\Theta|\le 1$ on $\Omega$ force $\Theta$ to be constant by the maximum modulus principle, contradicting $\Theta(\sigma+it)\to -1$ as $\sigma\to+\infty$. Hence $\xi$ has no zeros in $\Omega$.
\end{corollary}
