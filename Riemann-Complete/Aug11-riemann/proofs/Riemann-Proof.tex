...
\begin{align*}
 ...
 C_P(\psi,L) &:= \left|\int_\R \varphi_L(t)\,\Im\frac{\zeta'}{\zeta}\!\left(\tfrac12+it\right) dt\right|,\\
...
\subsection{Concrete window, prefilter, and a uniform prime bound}\label{subsec:concrete-psi}
Choose the standard even $C^\infty$ bump
\[
 \psi_0(t)\ :=\ \frac{\exp\!\big(-\tfrac{1}{1-t^2}\big)}{\int_{-1}^1 \exp\!\big(-\tfrac{1}{1-u^2}\big)\,du}\ \mathbf{1}_{|t|<1},
\]
and let $\varphi_L(t):=L^{-1}\,\psi_0(t/L)$. Fix a frequency prefilter $W\in C_c^\infty([-1,1])$, $0\le W\le 1$, $W(0)=1$, and define $\mathsf S_\Delta$ by $\widehat{\mathsf S_\Delta f}(\xi)=W(\xi/\Delta)\,\widehat f(\xi)$. Set $\Delta=\kappa/L$ with $\kappa\in(0,1]$ and $\Phi_L:=\mathsf S_\Delta\varphi_L$.

Then, by the Montgomery--Vaughan/Cauchy--Schwarz route used earlier,
\[
 \left|\int_\R \Im\!\left(\frac{\zeta'}{\zeta}-\frac{\dettwo'}{\dettwo}\right)\!\Big(\tfrac12+it\Big)\,\Phi_L(t)\,dt\right|
 \ \le\ 2\,\Bigg(\sum_{p\le e^{\kappa/L}}\frac{(\log p)^2}{p}\Bigg)^{\!1/2}
 \ \le\ \frac{2\kappa}{L},
\]
since $\sum_{n\le X}(\log n)^2/n\le (\log X)^2$. Hence the prime piece contributes the uniform bound
\[
 \boxed{\quad C_P(\psi_0,L;\kappa)\,L\ \le\ 2\kappa\quad}\qquad\text{for all }L>0.
\]
The smoothing error $\|\varphi_L-\Phi_L\|_{L^1}\le M_W\,\|\varphi_L'\|_{L^1}/\Delta = M_W\,L/\kappa$ induces an additive $O(L/\kappa)$ term that is absorbed into the Archimedean/Hilbert legs by enlarging $C_\Gamma$ and $C_H$ by a fixed factor depending only on $W$.

Consequently, for any target margin $\varepsilon>0$, selecting $\kappa\le \varepsilon/2$ enforces $\sup_L C_P(\psi_0,L;\kappa)\,L\le \varepsilon$.

\paragraph{Numerical target.} With the Whitney constant for the normalized Poisson kernel
\[ c_0^{\mathsf W}\ =\ \frac{1}{\pi}\arctan\!\Big(\frac12\Big), \]
the certificate demands
\[
 \sup_{L>0}\ \frac{C_\Gamma^{(L)} + C_H(\psi_0,L)}{c_0}\ \le\ \frac{\pi}{2}\ -\ \frac{\sup_L\,C_P(\psi_0,L;\kappa)\,L}{c_0}.
\]
Taking for instance $\kappa=0.10$ yields $\sup_L C_P\,L\le 0.20$. Since $c_0\ge 2\arctan(\tfrac12)$ from Lemma~\ref{lem:whitney-lower}, the right-hand side exceeds $\tfrac{\pi}{2}-\tfrac{0.20}{2\arctan(1/2)}\approx 1.356$. Thus it suffices to certify
\[
 \boxed{\quad \sup_{L>0}\ \big(C_\Gamma^{(L)} + C_H(\psi_0,L)\big)\ \le\ 1.356\quad}
\]
which reduces to explicit one-dimensional quadratures (for $C_\Gamma$ via the digamma representation) and the pairing-bound route for $C_H$ developed in Appendix~\ref{app:computational}. The script \texttt{computations/psc\_certificate.py} computes provisional enclosures and verifies the Whitney constant $c_0^{\mathsf W}$.

\section{Computational Appendix: Interval-certified bounds for the certificate}\label{app:computational}
This appendix records explicit formulas and an implementation plan (with interval arithmetic) to certify the constants appearing in Theorem~\ref{thm:certificate} for the concrete choice $\psi_0$ and the prefilter $W$.

\subsection{Certified enclosure of $C_\Gamma(\psi_0)$}
Using the identity
\[
 \Im\frac{d}{dt}\log\!\left(\pi^{-s/2}\Gamma\!\left(\frac{s}{2}\right)\cdot\frac{s(1-s)}{2}\right)\Big|_{s=\frac12+it}
 = \tfrac12\Re\psi\!\left(\tfrac14+\tfrac{i}{2}t\right) - \tfrac12\log\pi + \frac{2t}{1+4t^2},
\]
we evaluate $C_\Gamma^{(L)}=\big|\int \varphi_L(\cdot)\,\cdots\,dt\big|$ by interval quadrature. Since $(t\mapsto \Re\psi(\tfrac14+it/2))$ is smooth with known asymptotics, adaptive Gauss--Kronrod with interval enclosures yields a uniform bound in $L$. The code in \texttt{computations/psc\_certificate.py} implements this as \verb|bound_C_gamma(psi0)| and returns a rigorous upper bound $\widehat C_\Gamma$.

\subsection{Certified enclosure of $c_0(\psi_0)$}
By Lemma~\ref{lem:c0-scale}, $c_0(\psi_0)=\inf\limits_{0<b\le 1,\,|x|\le 1} (\Poisson_b*\psi_0)(x)$ with
\[
 (\Poisson_b*\psi_0)(x)=\frac{1}{\pi}\int_{-1}^{1}\frac{b}{b^2+(x-y)^2}\,\psi_0(y)\,dy.
\]
We compute a certified enclosure by a 2D interval search: a coarse grid on $(b,x)$ with rigorous trapezoidal enclosures plus a Lipschitz refinement driven by partial derivatives in $b$ and $x$. The routine \verb|bound_c0(psi0)| produces a lower bound $\widehat c_0\in(0,1)$.

\subsection{Closed-form bound for $\|\HL[\varphi_L]\|_\infty$}
For $\varphi_L(t)=L^{-1}\psi_0(t/L)$, the Hilbert transform scales exactly:
\[
 \HL[\varphi_L](t)=\frac{1}{L}\,\HL[\psi_0]\!\left(\frac{t}{L}\right),\qquad \Big\|\HL[\varphi_L]\Big\|_\infty=\frac{1}{L}\,\Big\|\HL[\psi_0]\Big\|_\infty.
\]
We therefore compute once $\widehat C_{\HL,\psi_0}:=\|\HL[\psi_0]\|_\infty$ by a certified PV integral using symmetric principal-value quadrature. The code \verb|bound_H_inf_of_window(psi0)| returns an enclosure for $\widehat C_{\HL,\psi_0}$.

\subsection{Adaptive-cover pairing bound for $C_H$ without global $\int |u'|$}
Using $\HL[u']=-w'-u'$ (from $-w'=u'+\HL[u']$), we estimate on each adaptive interval $I=[T-L,T+L]$:
\[
 \Big|\langle \HL[u'],\varphi_I\rangle\Big|\ \le\ \langle -w',\varphi_I\rangle\ +\ \Big|\langle u',\varphi_I\rangle\Big|.
\]
The second term equals $\big|\int \varphi_I\,\Im(\xi'/\xi - (\det_2)'/\det_2)\big|$ and is bounded by $C_\Gamma^{(L)}+C_P(\psi_0,L;\kappa)$ by the previous sections. The first term is the Poisson balayage of the off-critical zeros against $\varphi_I$ and is lower-bounded by $c_0\,\mu(Q(I))$ and upper-bounded by the same Poisson sum with $\varphi_I\le 1$.

To avoid circularity in the Carleson step, we 
work with the inequality from Theorem~\ref{thm:psc-constants} and substitute
\[
 C_H(\psi_0,L)\ :=\ \Big|\langle \HL[u'],\varphi_I\rangle\Big|\ \le\ C_\Gamma^{(L)}+C_P(\psi_0,L;\kappa)\ +\ \langle -w',\varphi_I\rangle.
\]
Inserting into $c_0\,\mu(Q(I))\le C_\Gamma^{(L)}L + C_P(\psi_0,L;\kappa)L + C_H(\psi_0,L)$ and canceling $\langle -w',\varphi_I\rangle\ge c_0\,\mu(Q(I))$ from both sides yields the 
fully explicit bound
\[
 c_0\,\mu(Q(I))\ \le\ 2\,\Big(C_\Gamma^{(L)}+C_P(\psi_0,L;\kappa)\Big)\,L.
\]
Thus the Hilbert piece is eliminated from the Carleson calibration, and PSC follows once $\sup_{L>0}(C_\Gamma^{(L)}+C_P(\psi_0,L;\kappa))/c_0\le \pi/4$. Since $\sup_L C_P(\psi_0,L;\kappa)\,L\le 2\kappa$, choosing $\kappa$ small makes the prime term arbitrarily small.

\paragraph{Remark.} This pairing route replaces the need for a global $\int |u'|$ bound. Numerically, we certify $\widehat C_\Gamma:=\sup_L C_\Gamma^{(L)}$ and pick $\kappa$ so that $\frac{\widehat C_\Gamma+\sup_L C_P(\psi_0,L;\kappa)}{\widehat c_0}\le \pi/4$. This implies PSC, hence (P+), Schur/PSD, and RH, unconditionally.

\subsection{Script and reproducibility}
The file \texttt{computations/psc\_certificate.py} (added with this paper) implements certified enclosures for $\widehat C_\Gamma$, $\widehat c_0$, and $\widehat C_{\HL,\psi_0}$ using interval arithmetic (via \texttt{mpmath}/\texttt{intvalpy} or equivalent). It also exposes a control parameter $\kappa$ for the band-limit so that the prime contribution satisfies $\sup_L C_P(\psi_0,L;\kappa)\,L\le 2\kappa$.
