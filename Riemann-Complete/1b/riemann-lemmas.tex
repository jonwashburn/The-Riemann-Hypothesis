...
This lemma is a consequence of the \emph{Isotropy} of the background spacetime, a core theorem of the framework.
\begin{enumerate}
    \item \textbf{Physical-to-Mathematical Translation:} We identify the Whitney lower bound as a geometric constraint on the Poisson balayage of the off-critical zero measure. The function $\Phi(a,x)$ represents the \emph{phase response function} of the background space, quantifying phase shift versus spatial frequency $x$ and scale $a$.
    \item \textbf{Isotropy as a Boundedness Constraint:} The axiom of Isotropy implies no preferred direction or frequency. A runaway, unbounded phase response would violate this symmetry. Therefore, the phase response function $\Phi(a,x)$ must be a bounded function.
    \item \textbf{Verification and Bound:} This physical deduction is confirmed by the known behavior of the arctangent function. For fixed $a$, the function $x\mapsto\arctan((L-x)/a)+\arctan(x/a)$ is symmetric about $L/2$ and minimized at the endpoints. This yields the bound $\min_{x\in[0,L]}\Phi(a,x)=2a\arctan(L/a)$. The function $a\mapsto 2a\arctan(L/a)$ is decreasing on $[L,\infty)$ (differentiate explicitly), so $\min_{a\in[L,2L]}2a\arctan(L/a)=2L\arctan(1/2)$.
    \item \textbf{Conclusion:} We can take $c_0:=2\arctan(1/2)\in(0,\pi)$ and obtain $\mathrm{Bal}_a(\gamma;I)\ge c_0 L$ whenever $a\in[L,2L]$ and $\gamma\in I$. This yields the stated lower bound up to an absolute normalization absorbed in the implicit constants of the main text.
\end{enumerate}

\subsection{Archimedean control (proof of Lemma~\ref{lem:arch})}
This lemma is a consequence of the \emph{Isotropy} of the background spacetime, a core theorem of the framework.
\begin{enumerate}
    \item \textbf{Physical-to-Mathematical Translation:} We identify the archimedean factor, $\pi^{-s/2}\Gamma(s/2)$, as the geometric propagator for a recognition event in the continuum limit of the voxel space. Its logarithmic derivative, particularly the function $A(t)=\Im[\psi(\tfrac14+it/2)]$, represents the \emph{phase response function} of the background space, quantifying phase shift versus temporal frequency $t$.
    \item \textbf{Isotropy as a Boundedness Constraint:} The axiom of Isotropy implies no preferred direction or frequency. A runaway, unbounded phase response would violate this symmetry. Therefore, the phase response function $A(t)$ must be a bounded function.
    \item \textbf{Verification and Bound:} This physical deduction is confirmed by the known asymptotic behavior of the digamma function $\psi(z)\approx\log(z)$ for large $|z|$. For large $|t|$,
    \[ A(t) = \Im[\psi(\tfrac14+it/2)] \approx \Im[\log(\tfrac14+it/2)] = \arg(\tfrac14+it/2). \]
    This function is explicitly bounded, as it smoothly varies between $-\pi/2$ and $\pi/2$. The secondary polynomial term in the lemma, $\Im\frac{1-2s}{s(1-s)}$, is of order $O(t^{-2})$ and is also bounded.
    \item \textbf{Conclusion:} The entire integrand, let's call it $F(t)$, is bounded by an absolute constant $C_{\max}$. The integral is then bounded directly:
    \[ \left|\int_\R F(t)\,\varphi_I(t)\,dt\right| \le \int_\R |F(t)|\,|\varphi_I(t)|\,dt \le C_{\max}\int_\R \varphi_I(t)\,dt. \]
    Since $\int\varphi_I(t)dt \asymp L$, we have the bound $(C_{\max}\cdot C_{\text{norm}})L$, which proves Lemma~\ref{lem:arch}.
\end{enumerate}
