...
\]
No bandlimit or prime budget enters the certificate; in the \(\zeta\)–normalized boundary gauge the \(k{=}1\) layer does not appear (no $C_P$ term). All inputs are unconditional; outward rounding policy is consolidated in Appendix H.
\end{abstract}
...
\[
U_{\mathrm{bal}}(s) := \Re\log\det\nolimits_2\!\bigl(I-\mathsf T(s)\bigr)-\Re\log\xi(s) = -\Re L(s).
\]
...
\[ c_0(\psi)\,\mu\big(Q(I)\big)\ \le\ C_\Gamma(\psi)+C_H(\psi)\ +\ C_{\mathrm{tail}}(\psi). \]
In particular, if \(\sup_{L>0}\frac{C_\Gamma(\psi)+C_H(\psi)}{c_0(\psi)}\le \pi/2\), then PSC holds.
\end{theorem}
\begin{proof}
